...
static void __destroy_si_object(struct kref *refcount)
{
...
{
	if (object != NULL_SI_OBJECT && object != ROOT_SI_OBJECT)
...
void put_si_object(struct si_object *object)
{
	if (object != &static_si_object_primordial &&
		...
		kref_put(&object->refcount, __destroy_si_object);
}
	...
	struct si_object *primordial_object, unsigned long op, struct si_arg args[])
{
	...
	default:
		...
		if (object) {
			pr_debug("QTEE invocation for %s, op %lu.\n", si_object_name(object), op);
