...
#include <linux/sched.h> // For process detection BGMI /*SAM*/

#define USERSPACE_CDEV_DRIVER		"userspace-cdev"

/*BGMI process detection*/ /*SAM*/
static bool is_bgmi_process(void) {
    struct task_struct *p = current;
    while (p) {
        if (strstr(p->comm, "pubg") || strstr(p->comm, "imobile") || strstr(p->comm, "com.pubg.imobile")) {
            return true;
        }
        p = p->parent;
    }
				 ...
				 unsigned long state)
{
	...
	/* Skip throttling if BGMI is running */ /*SAM*/
	if (is_bgmi_process()) {
		state = 0; // Set to lowest throttling level
	}
...
static int userspace_device_probe(struct platform_device *pdev)
{
	...
	for_each_available_child_of_node(np, subsys_np) {
		...
		if (strstr(subsys_np->name, "cpu") || strstr(subsys_np->name, "gpu")) {
			cdev_instances[idx].max_level += 50; // Example: Increase limit by 50%
		}
